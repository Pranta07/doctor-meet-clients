...
const DiagnosisPay = () => {
    ...
    const { premiumMemberDetails, premiumMembershipStatus } =
    ...
    useEffect(() => {
        const fetchData = async () => {
            await fetch(
                `https://floating-basin-02241.herokuapp.com/bookedDiagnosis/${params.id}`
            )
                .then((res) => res.json())
                .then((data) => {
                    setDiagnostic(data);
                    ...
                    if (premiumMembershipStatus) {
                        const membershipDiscountPercentage =
                            premiumMemberDetails?.categoryDetails
                                .labTestDiscount;
                        setLastPrice(
                            (
                                floatPrice -
                                floatPrice *
                                    parseFloat(
                                        membershipDiscountPercentage / 100
                                    )
                            ).toFixed(0)
                        );
                        seteMembershipDiscountPrice(
                            (
                                floatPrice *
                                parseFloat(membershipDiscountPercentage / 100)
                            ).toFixed(0)
                        );
                    }
                });
                ...
                {premiumMembershipStatus ? (
                    <div>
                        ...
                        <div className="d-flex justify-content-center align-items-center">
                            ...
                            />
                            <h3 className="text-right">
                                BDT {parseInt(membershipDiscountPrice) * 100}
                            </h3>
                        </div>

                        <hr
                            className="w-100 my-3 mx-auto"
                            style={{ border: "2px solid #c8d6e5" }}
                        />
                        ...
                        <div className="d-flex justify-content-center align-items-center">
                            ...
                            />
                            <h3 className="text-right">
                                BDT {parseInt(lastPrice).toFixed(0) * 100}
                            </h3>
                        </div>
                    </div>
                ) : (
