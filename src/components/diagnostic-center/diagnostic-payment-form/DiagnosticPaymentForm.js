...
const DiagnosticPaymentForm = ({ diagnostic }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            await setFloatPrice(
                diagnostic?.selectedDiagnosis.price -
                    diagnostic?.selectedDiagnosis.price *
                        (parseFloat(
                            diagnostic?.selectedDiagnosis.discount
                        ).toFixed(2) /
                            100.0)
            ).toFixed(2);
            ...
            if (premiumMembershipStatus) {
                const membershipDiscountPercentage =
                    premiumMemberDetails?.categoryDetails.labTestDiscount;
                setFloatPrice(
                    floatPrice -
                        floatPrice *
    ...
    const handleSubmit = async (event) => {
        ...
        if (error) {
            ...
            if (paymentMethod.id) {
                fetch(
                    `https://floating-basin-02241.herokuapp.com/bookedDiagnosis/${diagnostic._id}`,
                    {
                        method: "PUT",
                    }
                )
    ...
    return (
        <form onSubmit={handleSubmit} className="my-5">
            ...
            <button
                type="submit"
                className="btn-diagnosis-pay mt-5"
                disabled={!stripe}
            >
                Pay
            </button>
